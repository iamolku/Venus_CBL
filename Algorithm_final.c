// Final iteration of the Group 16 algorithm
...
int vl53l0x_example_single(void) {
	int i;
	gpio_set_level(IO_AR13, GPIO_LEVEL_HIGH);
	uint8_t addr = 0x29;
	i = tofPing(IIC0, addr);
	printf("Sensor Ping: ");
	if(i != 0)
    ...
    sleep_msec(500);
	for (i=0; i<10; i++) // read values 10 times half a second
	{
...
int explore(matrix_t matrix, int orientation, int prev_d_i, int exp_fin, pos currpos, pos prevpos ){
    if(exp_fin!=1){
       ...
       */
        ...
        if(curr_topd_i <150 && curr_topd_i>100){ //if cliff, crater, nothing or potentially mountain
            if (c== 'l' || (curr_topd_i > 120)){ // if cliff or crater
        ...
        else if(curr_topd_i < 90 && curr_topd_i>80){
        ...
        else if(curr_topd_i <75 && curr_topd_i >65){
